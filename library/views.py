...
def viewissuedbook_view(request):
    ...
    for ib in issuedbooks:
        ...
        i=0
        for l in books:
            t=(students[i].get_name,students[i].enrollment,books[i].name,books[i].author,issdate,expdate,update)
            i=i+1
            li.append(t)
...
def contactus_view(request):
    ...
    if request.method == 'POST':
        ...
        if sub.is_valid():
            ...
            send_mail(str(name)+' || '+str(email),message, EMAIL_HOST_USER, ['hrishabhsingh4n@gmail.com'], fail_silently = False)
